...
class MainWindow(QMainWindow):
    ...
    def show_start_screen(self):
        ...
        card = QWidget()
        card.setFixedSize(int(self.screen.width() * 0.25),
        ...
        bearing_btn.setStyleSheet(
            "background-color:#3498db;border-radius:20px;color:white;"
        )
        ...
        yataqan_btn.setStyleSheet(
            "background-color:#9b59b6;border-radius:20px;color:white;"
        )
    ...
    def show_bearing_ui(self):
        self._show_bearing_or_yataqan(
            inputs=["حلقه بیرون", "حلقه درون", "ضخامت"]
        )

    def show_yataqan_ui(self):
        self._show_bearing_or_yataqan(
            inputs=["اندازه داخلی"], single_input=True
        )
    ...
    def _show_bearing_or_yataqan(self, inputs, single_input=False):
        ...
        main_v.setAlignment(Qt.AlignCenter)
        main_v.setSpacing(0)
        ...
        if single_input:
            ...
            box_height = int(((card_height - 50) // n) * 0.9)
        ...
        btn_card = QWidget()
        btn_card.setFixedSize(card_width * 2 + main_h.spacing(), 80)
    ...
    def check_result(self):
        values = [i.text() for i in self.inputs if i.text()]
        if not values:
